...
contract EvidenceController{
    ...
    function createSaveRequest(bytes32 hash, bytes32[] memory ext) public validateHash(hash){
    ...
    function voteSaveRequest(bytes32 hash) public validateHash(hash) returns(bool){
        ...
        (bytes32 h, address creator, bytes32[] memory ext,  uint8 voted, uint8 threshold) =  _requestRepo.getRequestData(hash);
        bool passed = voted >= threshold;
        emit VoteSaveRequest(hash, msg.sender, passed);
        if(passed){
            _evidenceRepo.setData(hash, creator, now, ext);
    ...
    function getEvidence(bytes32 hash) public view returns(bytes32 , address, uint, bytes32[]){
