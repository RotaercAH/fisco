...
contract RewardPointController is BasicAuth {
    ...
    event LogSend( address indexed from, address indexed to, bytes32[] value);
    //bytes32[] zero = 0x111122223333444455556666777788889999AAAABBBBCCCCDDDDEEEEFFFFCCCA;
    bytes32[] zero;
    ...
    modifier canUnregister(address account) { 
        require(_rewardPointData.hasAccount(account)==true && _rewardPointData.getBalance(account).length == 0 , "Cann't unregister!");
        _; 
    }
    /*modifier canUnregister(address account) { 
        require(_rewardPointData.hasAccount(account)==true, "Cann't unregister!");
        _; 
    }*/
    ...
    function getbytes(bytes32[] test) public{
       zero = test;
   }
    ...
    function register() accountNotExist(msg.sender) public returns (address) {
        ...
        _rewardPointData.setBalance(msg.sender, zero);
    ...
    function balance(address addr) public view returns (bytes32[]) {
        return _rewardPointData.getBalance(addr);
    }
    /*
  ...
  */
  /*  function issue(address account, uint256 value) public accountExist(account) returns (bool) {
    ...
    function issue(address account, bytes32[] value) public accountExist(account) returns (bool) {
        /*uint256 totalAmount = _rewardPointData._totalAmount();
        totalAmount = totalAmount.add(value);
        _rewardPointData.setTotalAmount(totalAmount);
        uint256 balance1 = _rewardPointData.getBalance(account);
        balance1 = balance1.add(value);*/
        _rewardPointData.setBalance(account, value);
        emit LogSend( address(0), account, value);
        return true;
    }
